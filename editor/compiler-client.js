...
export function createCompilerClient(compilerSource, assetBase, environment = globalThis) {
  ...
  return async function compile(source, options = {}) {
    ...
    if (!available || failed) {
      if (!fallback) {
        const moduleURL = environment.URL.createObjectURL(new environment.Blob([
          compilerSource, '\nexport const compile = TaildownCompiler.compile; export const setAssetBase = TaildownCompiler.setAssetBase;',
      ...
      return (await fallback).compile(source, snapshot);
    }
    return new Promise((resolve, reject) => {
      const id = ++sequence;
      pending.set(id, {resolve, reject});
      try { worker.postMessage({id, source, options: snapshot, assetBase}); }
