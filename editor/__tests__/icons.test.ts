...
function setup() {
  ...
  const editor = new Function('Taildown', 'document', registry + autocomplete + '\nreturn {iconNames, taildownAutocomplete, renderIconCompletion};')(icons, document);
...
it('uses the complete compiler registry for editor icon suggestions', () => {
  ...
  const result = editor.taildownAutocomplete({pos: input.length, matchBefore: () => null, state: {doc: {lineAt: () => ({text: input, from: 0})}}});
