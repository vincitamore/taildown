  ...
  const result = await compile('# Title {editorial small}', {
    components: {
      editorial: {
        name: 'editorial',
        htmlElement: 'article',
        defaultClasses: ['font-bold', 'text-4xl'],
      },
    },
  });
  const dom = new JSDOM(result.html);
...
it('keeps conflicting definitions isolated between concurrent compiles and subsequent parses', async () => {
  ...
  const results = await Promise.all(
    definitions.map((htmlElement) =>
      compile(source, {
        components: {
          editorial: {
            name: 'editorial',
            htmlElement,
            defaultClasses: [htmlElement === 'article' ? 'p-4' : 'p-8'],
          },
        },
      })
    )
  ...
  results.forEach((result, i) => {
    expect(result.html).toContain(
      `<${definitions[i]} class="taildown-component component-editorial`
    );
    expect(result.metadata.warnings).toEqual([]);
  });
  expect(
    (await parseWithWarnings(source)).warnings.some(
      (w) => w.message === 'Unknown component: editorial'
    )
  ).toBe(true);
});
...
it('rejects ambiguous component definitions instead of silently altering built-in behavior', async () => {
  await expect(
    compile('', { components: { card: { name: 'card', defaultClasses: [] } } })
  ).rejects.toThrow('conflicts with a registered component');
  await expect(
    compile('', { components: { editorial: { name: 'other', defaultClasses: [] } } })
  ).rejects.toThrow('key and name must match');
  await expect(
    compile('', {
      components: {
        editorial: { name: 'editorial', htmlElement: 'article onclick=x', defaultClasses: [] },
      },
    })
  ).rejects.toThrow('Invalid HTML element');
});
...
it('snapshots caller definitions and vocabulary before asynchronous initialization', async () => {
  ...
  const pending = compile(':::editorial{brand}\nOriginal\n:::', {
    components: { editorial: definition },
    styleMappings,
  });
...
it('surfaces the same custom vocabulary and insertable examples to authoring clients', async () => {
  const options = {
    components: {
      editorial: { name: 'editorial', htmlElement: 'article', defaultClasses: ['p-4'] },
    },
    styleMappings: { brand: 'text-sm' },
  };
  ...
  expect(
    (await getAuthoringReference()).components.some((component) => component.name === 'editorial')
  ).toBe(false);
});
...
it.each([
  ...
  'rejects malformed custom definitions consistently in compiler and authoring reference: %j',
  async (components) => {
    const options = { components };
    await expect(Reflect.apply(compile, undefined, ['', options])).rejects.toThrow(
      /Custom component|Invalid HTML element/
    );
