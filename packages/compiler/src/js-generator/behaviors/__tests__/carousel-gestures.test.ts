...
it('distinguishes horizontal swipes from scrolling, cancellation and multiple touches', async () => {
  ...
  try {
    dom.window.eval(
      'function getComponents(name){return document.querySelectorAll("[data-component="+name+"]")} function toggleClass(el,name,on){el.classList.toggle(name,on)}\n' +
    ...
    const send = (type: string, touches: ReturnType<typeof point>[], changedTouches = touches) => {
      carousel.dispatchEvent(
        Object.assign(new dom.window.Event(type, { bubbles: true }), { touches, changedTouches })
      );
    };
    const current = () =>
      carousel.querySelector('[data-carousel-slide]:not([hidden])')!.textContent.trim();
    ...
    carousel.dispatchEvent(
      new dom.window.MouseEvent('mousedown', { button: 0, screenX: 200, bubbles: true })
    );
