...
it('treats error and destructive as the same button variant in compilation and authoring references', async () => {
  ...
  try {
    ...
    expect(
      reference.components.find((component) => component.name === 'button')?.attributes
    ).toEqual(expect.arrayContaining(['error', 'destructive']));
...
it('keeps alert text readable against its translucent surface in either theme', async () => {
  ...
  for (const name of ['success', 'warning', 'error', 'info']) {
    const colors = [...palette.matchAll(new RegExp(`--${name}-text:\\s*([^;]+);`, 'g'))].map(
      (match) => match[1]!
    );
    const rules = [...result.css.matchAll(new RegExp(`\\.alert-${name} \\{([^}]+)\\}`, 'g'))].map(
      (match) => match[1]!
    );
    expect(rules).toHaveLength(2);
    [getLightModeColors(config), getDarkModeColors(config)].forEach((theme, index) => {
      expect(rules[index]).toContain(`color: var(--${name}-text)`);
      const rgba = rules[index]!.match(/background: rgba\(([^)]+)\)/)![1]!
        .split(',')
        .map(Number);
      for (const base of [theme.background, theme.card]) {
        ...
        const blended =
          ...
          channels
            .map((value, channel) =>
              Math.round(rgba[channel]! * rgba[3]! + value * (1 - rgba[3]!))
                .toString(16)
                .padStart(2, '0')
            )
            .join('');
        expect(
          contrast(colors[index]!, blended),
          `${name} alert ${index} over ${base}`
...
it('provides readable status text and status background pairs in both themes', async () => {
  ...
  for (const name of names) {
    const shades = [...css.matchAll(new RegExp(`--${name}-text:\\s*([^;]+);`, 'g'))].map(
      (match) => match[1]!
    );
    expect(shades).toHaveLength(2);
    [getLightModeColors(config), getDarkModeColors(config)].forEach((theme, index) => {
      expect(
        contrast(shades[index]!, theme.background),
        `${name} page ${index}`
      ).toBeGreaterThanOrEqual(4.5);
      expect(contrast(shades[index]!, theme.card), `${name} card ${index}`).toBeGreaterThanOrEqual(
        4.5
      );
      expect(
        contrast(theme[name], theme[`${name}Foreground`]),
        `${name} pair ${index}`
      ).toBeGreaterThanOrEqual(4.5);
    });
  }
  const result = await compile(
    names
      .map(
        (name) =>
          `Text ${name} {${name}}\n\n:icon[check]{${name}}\n\nPair ${name} {${name}-bg}\n\n[Action ${name}](#){button ${name === 'error' ? 'destructive' : name}}`
      )
      .join('\n\n') +
      '\n\nLarge {large-warning}\n\nFixed {text-yellow-600}\n\nOverride {warning-bg text-white}',
    { inlineStyles: true }
  );
  const dom = new JSDOM(result.html);
  try {
    ...
    for (const name of names) {
      ...
      expect(document.querySelector(`p.bg-${name}.text-${name}-foreground`)?.textContent).toBe(
        `Pair ${name}`
      );
    ...
    expect(
      document
        .querySelector('p.bg-warning.text-white')
        ?.classList.contains('text-warning-foreground')
    ).toBe(false);
    expect(result.metadata.warnings).toEqual([]);
  } finally {
    dom.window.close();
...
it('uses semantic text tokens across headings, prose, icons and components while keeping numbered overrides literal', async () => {
  ...
  try {
    ...
    expect(result.css).toContain(
      '.hover\\:text-primary-hover:hover { color: var(--primary-text-hover); }'
    );
    expect(result.metadata.warnings).toEqual([]);
  } finally {
    dom.window.close();
...
it('provides readable normal and hover semantic text on both default document surfaces', () => {
  ...
  for (const name of ['primary', 'secondary', 'accent']) {
    for (const suffix of ['text', 'text-hover']) {
      const shades = [...css.matchAll(new RegExp(`--${name}-${suffix}:\\s*([^;]+);`, 'g'))].map(
        (match) => match[1]!
      );
      expect(shades).toHaveLength(2);
      [getLightModeColors(config), getDarkModeColors(config)].forEach((theme, index) => {
        expect(
          contrast(shades[index]!, theme.background),
          `${name}/${suffix} background ${index}`
        ).toBeGreaterThanOrEqual(4.5);
        expect(
          contrast(shades[index]!, theme.card),
          `${name}/${suffix} card ${index}`
        ).toBeGreaterThanOrEqual(4.5);
      });
    }
  }
...
it('gives inline code a readable foreground/background pair in both themes', () => {
  ...
  const values = (name: string) =>
    [...css.matchAll(new RegExp(`--inline-code-${name}:\\s*([^;]+);`, 'g'))].map(
      (match) => match[1]!
    );
  ...
  text.forEach((color, index) =>
    expect(contrast(color, backgrounds[index]!)).toBeGreaterThanOrEqual(4.5)
  );
});
...
it('keeps muted text readable on both page and card surfaces', async () => {
  ...
  try {
    expect(
      [...dom.window.document.querySelectorAll('p.text-muted-foreground')].map((n) => n.textContent)
    ).toEqual(['Plain', 'Large', 'Small', 'Bold', 'Italic']);
