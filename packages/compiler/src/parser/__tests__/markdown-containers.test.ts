...
it.each(['\n', '\r\n'])('keeps cards inside quotes and list items (%j)', async newline => {
  for (const [lines, selector] of [
    ...
    const nodes: any[] = [];
    const walk = (node: any) => { if (node.type === 'containerDirective') nodes.push(node); node.children?.forEach(walk); };
    walk(ast);
...
it('does not let unclosed components consume a sibling list item', async () => {
  ...
  expect(ast.children[0]).toMatchObject({type: 'list', children: [
    {type: 'listItem', children: [{type: 'containerDirective', children: [{children: [{value: 'Inside'}]}]}]},
...
it.each([
  ...
  expect(warnings).toEqual([]);
  const nodes: any[] = [];
  const walk = (node: any) => { if (node.type === 'code') nodes.push(node); node.children?.forEach(walk); };
  walk(ast);
