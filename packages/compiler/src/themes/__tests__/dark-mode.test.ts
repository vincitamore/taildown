...
it('compiled portable HTML creates a working toggle when storage is blocked', async () => {
  ...
  const dom = new JSDOM(result.html,{runScripts:'dangerously',beforeParse(window){
    Object.defineProperty(window,'localStorage',{get(){throw new window.DOMException('Unavailable','SecurityError');}});
  }});
  try {
...
it('works without storage and retains the user choice across system changes', async () => {
  ...
  try {
    ...
    Object.defineProperty(window,'localStorage',{get(){throw new window.DOMException('Unavailable','SecurityError');}});
    window.matchMedia = (()=>({matches:true,addEventListener(_type:string,listener:typeof systemChange){systemChange=listener;}})) as any;
    expect(()=>window.eval(generateDarkModeScript(getDarkModeOptions(getDefaultConfig())))).not.toThrow();
...
it('ignores invalid stored preferences and follows the system until explicitly toggled', async () => {
  ...
  try {
    ...
    let systemChange = (_event:{matches:boolean}) => {};
    window.matchMedia = (()=>({matches:false,addEventListener(_type:string,listener:typeof systemChange){systemChange=listener;}})) as any;
