...
function run(code:string): unknown {
 const script=`import assert from 'node:assert/strict';import * as api from ${JSON.stringify(pathToFileURL(bundle).href)};try{const value=await (async()=>{${code}})();console.log(JSON.stringify({value}));}catch(error){console.log(JSON.stringify({error:error.message}));}`;
 const result: unknown=JSON.parse(execFileSync(process.execPath,['--input-type=module','-'],{input:script,encoding:'utf8'}));
 if(typeof result !== 'object' || result === null)throw new Error('Invalid subprocess result');
 if('error' in result){if(typeof result.error !== 'string')throw new Error('Invalid subprocess error');throw new Error(result.error);}
 if(!('value' in result))throw new Error('Missing subprocess value');
 return result.value;
}
const load=(options:object, checks='')=>run(`const result=await api.loadConfig(${JSON.stringify(options)});${checks}return result;`);
it.each(['mjs','cjs'])('loads partial %s settings relative to cwd and preserves inherited defaults',async ext=>{
 ...
 const result=load({cwd:dir,configPath:'settings.'+ext,throwOnError:true},'assert.ok(result.config.theme.fonts.sans);');
 expect(result).toMatchObject({warnings:[],hasUserConfig:true,config:{theme:{colors:{primary:{DEFAULT:'#123456'}}},output:{minify:true}}});
});
it('discovers a partial config without treating it as an invalid complete theme',async()=>{
 ...
 const result=load({cwd:dir,throwOnError:true});expect(result).toMatchObject({config:{theme:{glass:{opacity:42}}},warnings:[]});
});
it('rejects invalid merged settings or falls back with an explicit diagnostic',async()=>{
 ...
 const fallback=load({configPath:file},"assert.ok(result.warnings.join(' ').includes('Glass opacity'));assert.notEqual(result.config.theme.glass.opacity,140);");expect(fallback).toMatchObject({hasUserConfig:false});
});
it('returns isolated defaults on both missing and failed loads',async()=>{
 ...
 const result=run(`const options=${JSON.stringify({cwd:dir})};const first=await api.loadConfig(options);const original=first.config.theme.colors.primary.DEFAULT;first.config.theme.colors.primary.DEFAULT='#000000';const second=await api.loadConfig(options);const failed=await api.loadConfig(${JSON.stringify({configPath:join(dir,'missing.mjs')})});failed.config.theme.colors.primary.DEFAULT='#ffffff';assert.equal(second.config.theme.colors.primary.DEFAULT,original);assert.equal((await api.loadConfig(options)).config.theme.colors.primary.DEFAULT,original);return true;`);
