...
function normalizeAST(value: unknown): unknown {
  ...
  return Object.fromEntries(
    entries
      .filter(
        ([key, child]) =>
          !(key === 'position' && 'type' in value && typeof value.type === 'string') &&
          !(
            key === 'attributes' &&
            child &&
            typeof child === 'object' &&
  ...
  .flatMap((category) =>
    readdirSync(join(root, category.name))
      ...
      .map((name) => ({
        ...
        path: join(root, category.name, name),
        base: join(root, category.name, basename(name, extname(name))),
      }))
  );
...
const renderedFixtures = new Map<string, (source: string) => Promise<void>>([
  [
    '10-content-components/07-table-enhanced.td',
    async (source) => {
      ...
      expect(
        [...tables[1]!.querySelectorAll('tbody tr')].map((row) => row.lastElementChild?.textContent)
      ).toEqual(['$85,000', '$75,000', '$120,000']);
...
describe('Syntax fixture conformance', () => {
  it('discovers fixtures and requires an executable expectation for every input', () => {
    ...
    for (const fixture of fixtures) {
      expect(
        existsSync(fixture.base + '.ast.json') || renderedFixtures.has(fixture.name),
        fixture.name
      ).toBe(true);
    }
    for (const name of renderedFixtures.keys())
      expect(
  ...
  for (const fixture of fixtures.filter((fixture) => existsSync(fixture.base + '.ast.json'))) {
    it(fixture.name, async () => {
      const expected: unknown = JSON.parse(readFileSync(fixture.base + '.ast.json', 'utf8'));
  ...
  it('compares semantic attributes while ignoring empty attributes and source positions', () => {
    expect(
      normalizeAST({ type: 'containerDirective', attributes: {}, position: { start: 1 } })
    ).toEqual({ type: 'containerDirective' });
    expect(normalizeAST({ attributes: { id: 'help', columns: '3' } })).not.toEqual(
      normalizeAST({ attributes: { id: 'other', columns: '2' } })
    );
    expect(normalizeAST({ attributes: { position: 'top' } })).toEqual({
